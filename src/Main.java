...
public class Main {
    public static void main(String[] args) {
        ...
        Map<String, Integer> dem = demSoLanXuatHien(cau);
        for (String tu : dem.keySet()) {
            System.out.println(tu + ": " + dem.get(tu) + " lần");
        }
    }

    public static Map<String, Integer> demSoLanXuatHien(String cau) {
        Map<String, Integer> dem = new HashMap<>();
        String[] cacTu = cau.split("\\s+");
        for (String tu : cacTu) {
            String tuChuanHoa = tu.replaceAll("[^a-zA-Z0-9]", "").toLowerCase();
            dem.put(tuChuanHoa, dem.getOrDefault(tuChuanHoa, 0) + 1);
        }
        return dem;
    }
}
